...
[t,x]=ode45('ode_solver_script_q1_b',[0,10],[pi/4,0,pi/4,0]);
...
for i=1:1:length(x)
    ...
    subplot(222)
    plot(t(i,1),theta1,'or','LineWidth',1)
    hold on
    title('$\theta_{1}$','Interpreter','latex')
    xlabel('Time (s)','Interpreter','latex')
    ylabel('Energy (J) ','Interpreter','latex')
    set(gca,'FontSize',18)
    grid minor
    ...
    subplot(224)
    plot(t(i,1),theta2,'ob','LineWidth',1)
    hold on
...
writerObj = VideoWriter('animation_Q1_b.avi');
